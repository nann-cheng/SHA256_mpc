...
// From message lengh (in bits), compute the padding bits up to a whole block (512 bits)
 pub fn get_padded_bits( bytesLen:usize ) -> Vec<bool> {
    ...
    assert!(( padding.len()*8) % 512 == 0, "Padding did not complete properly!");

    convertBytes2Bits(&padding)
}
...
pub fn plaintext_block_eval(circuit:&mut Sha256Circuit, evaluation_map:&mut HashMap<usize, PlainEvalWire> ,output_bits:&mut Vec<bool>){
...
pub fn planitext_eval(message:&Vec<u8>, circuit:&mut Sha256Circuit) -> Vec<u8> {
    ...
    let secret_bits = convertBytes2Bits(message);
    ...
    for i in 0..block_cnt{
        ...
        for j in 0..STATE_INFO_BITS_LEN{
            ...
            if i==0{
                evaluation_map.insert( state_wire_id, PlainEvalWire{val: circuit.get_initial_hash(j), flipped: false} );
            }else{//accept internal output wire result
                evaluation_map.insert( state_wire_id, PlainEvalWire{val: last_evaluation_result[j], flipped: false} );
            }
        }
        ...
        for j in left_start_index..right_end_index{
            ...
            if j < secret_bits_cnt {//real secret bits input
                ...
                evaluation_map.insert(gate.input0, PlainEvalWire{ val:secret_bits[j], flipped: false} );//party-0 input
                evaluation_map.insert(gate.input1, PlainEvalWire{ val: false, flipped: false} );//party-1 input
                extra_double_gates.push(gate);
            }else{
                //padding bits
                evaluation_map.insert(circuit_wire_id, PlainEvalWire{val: overall_bits[j], flipped: false}  );//party-1 input
            }
        }
